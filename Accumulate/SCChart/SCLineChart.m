...
//    }
    ...
    //画横线
    for (int i=0; i<rowCount+1; i++) {
        if ([_ShowHorizonLine[i] integerValue]>0&&(i==0||i==rowCount)) {
            
            CAShapeLayer *shapeLayer = [CAShapeLayer layer];
            UIBezierPath *path = [UIBezierPath bezierPath];
            [path moveToPoint:CGPointMake(0,UULabelHeight+i*levelHeight)];//UUYLabelwidth
...
-(void)setXLabels:(NSArray *)xLabels
{
    ...
    _xLabelWidth = (self.frame.size.width)/num;//- UUYLabelwidth
    
    for (int i=0; i<xLabels.count; i++) {
        NSString *labelText = xLabels[i];
        SCChartLabel * label = [[SCChartLabel alloc] initWithFrame:CGRectMake(i * _xLabelWidth, self.frame.size.height - UULabelHeight, _xLabelWidth, UULabelHeight)];//UUYLabelwidth
    ...
    for (int i=0; i<xLabels.count+1; i++) {
        CAShapeLayer *shapeLayer = [CAShapeLayer layer];
        UIBezierPath *path = [UIBezierPath bezierPath];
        [path moveToPoint:CGPointMake(i*_xLabelWidth+_xLabelWidth/2,UULabelHeight)];//UUYLabelwidth+
        [path addLineToPoint:CGPointMake(i*_xLabelWidth+_xLabelWidth/2,self.frame.size.height-2*UULabelHeight)];
        [path closePath];//UUYLabelwidth+
        shapeLayer.path = path.CGPath;
        shapeLayer.strokeColor = [[[UIColor blackColor] colorWithAlphaComponent:0.1] CGColor];
        shapeLayer.fillColor = [[UIColor whiteColor] CGColor];
        shapeLayer.lineWidth = 1;
...
-(void)strokeChart
{
    for (int i=0; i<_yValues.count; i++) {
        ...
        CGFloat xPosition = (_xLabelWidth/2.0);//UUYLabelwidth +
        CGFloat chartCavanHeight = self.frame.size.height - UULabelHeight*3;
...
- (void)addPoint:(CGPoint)point index:(NSInteger)index isShow:(BOOL)isHollow value:(CGFloat)value
{
    ...
    UITapGestureRecognizer *tap=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(showAccessaryView:)];
    [view addGestureRecognizer:tap];
}
-(void)showAccessaryView:(UITapGestureRecognizer*)tap
{
